...
class MultiDomainEnv(object):
    ...
    def reset(self, start, ends):
        ...
        # 设置路由状态矩阵
        route_matrix = np.zeros((self.num, self.num), dtype=self.numpy_type)  # 设置一个全0的矩阵
        idx = utils.node_to_index(self.start)  # 因为矩阵和交换机的索引不一致，所以要进行转换
        route_matrix[idx, idx] = 1  # 设置起点为1
        idx_node = utils.node_to_index(self.ends)
        route_matrix[idx_node, idx_node] = -1  # 把终点设置为-1

        # 强化学习状态矩阵拼接
        # state = torch.stack([torch.from_numpy(route_matrix), torch.from_numpy(self.pkt_drop_matrix),
        #                      torch.from_numpy(self.network_fault_matrix),
        #                      torch.from_numpy(self.normal_free_bw_matrix), torch.from_numpy(self.normal_delay_matrix),
        #                      torch.from_numpy(self.normal_loss_matrix),
        #                      torch.from_numpy(self.normal_use_bw_matrix), torch.from_numpy(self.normal_pkt_err_matrix),
        #                      torch.from_numpy(self.normal_distance_matrix)
        #                      ], dim=1)
    ...
    def step(self, action, _state):
        ...
        # 对下一跳节点做个转换，变成索引值
        agent_index = np.argwhere((_state == 1) | (_state == 2)).flatten()[0].item()  # 获取智能体当前位置的索引
        # print(agent_index)
        ...
        if link_flag:  # 所选的动作在邻接边中的情况
            ...
            if not judge_flag:  # 如果没有抵达目的终点
                ...
                if action in self.intelligent_routing_path.get((self.start, self.ends))[:-1]:  # 链路存在环路的情况
                    ...
                    reward_score += 1  # 进行激励
                    # 同时判断当前状态是否为子目标状态
                    if utils.judge_subgoal_flag(_state):
                        route_done = False
                        reward_score = self.reward_single_link_score((agent_node, action))
                        reward_score += 5  # 给予子目标的奖励值
                        next_state = utils.update_state(agent_index, next_jump_index, _state)  # 更新下一个状态
                    else:
                        route_done = False
                        reward_score = 0  # 不是子目标不给予奖励值
                        next_state = utils.update_state(agent_index, next_jump_index, _state)  # 更新下一个状态
...
if __name__ == "__main__":
    ...
    for index, pkl_path in enumerate(test_dataset.pickle_file_path_yield()):
        print(pkl_path)
        if index == 1:
            a = test_env.read_pickle_and_modify(pkl_path)
            print(a)
            # test_env.reset(1, 29)

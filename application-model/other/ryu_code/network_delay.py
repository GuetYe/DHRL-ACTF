...
class NetworkDelay(app_manager.RyuApp):
  ...
  echo latency |
        ...
        # print("delay------echo_delay_table------>",self.echo_delay_table)
    ...
    def create_delay_graph(self):
        ...
        for src, dst in self.network_structure.graph.edges:
            delay = self.calculate_delay(src, dst) # 计算时延信息
            self.network_structure.graph[src][dst]['delay'] = delay # 将时延信息存储到图中

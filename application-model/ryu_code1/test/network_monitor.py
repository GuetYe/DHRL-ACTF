...
class NetworkMonitor(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # self.monitor_thread = hub.spawn(self._monitor)  # 启动协程
    ...
    def port_desc_stats_reply_handler(self, ev):
        ...
        for ofport in ev.msg.body:
            if ofport.port_no != ofproto_v1_3.OFPP_LOCAL:  # 0xfffffffe 4294967294
                ...
                # print("port_features---->", ofport.port_no)
    ...
    def port_stats_table_reply_handler(self, ev):
        ...
        for stat in sorted(body, key=attrgetter("port_no")):
            ...
            if port_no != ofproto_v1_3.OFPP_LOCAL:
                ...
                self._save_stats(self.port_stats_table, key, speed, 5)
                self._calculate_port_free_bandwidth(dpid, port_no, speed)
        # print("\n")
    ...
    def _calculate_port_free_bandwidth(self, dpid, port_no, speed):
        ...
        if port_features:
            ...
            # 当前带宽
            curr_bw = max(capacity / 10 * 3 - speed * 8 / 10 ** 9, 0)  # Mbit/s
            # print("curr_bw--------->", curr_bw)
    ...
    def create_bandwidth_graph(self):
        ...
        for link in link_port_table:
            ...
            if src_dpid in self.port_free_bandwidth.keys() and dst_dpid in self.port_free_bandwidth.keys():
                ...
                src_dst__bandwitdh = min(src_port_bw, dst_port_bw)
                # print("monitor------> src_dst_bandwitdh", src_dst_bandwitdh)

                # 对图的edge设置bw值
                self.network_structure.graph[src_dpid][dst_dpid]['bw'] = src_dst__bandwitdh
            else:
                # print("monitor----> not in port_free_bandwidth", src_dpid, dst_dpid)
                self.network_structure.graph[src_dpid][dst_dpid]['bw'] = 0
